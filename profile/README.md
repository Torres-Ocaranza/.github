<div align="center">

# 🏢 Torres Ocaranza

**Empresa líder en enfierradura, construcción y suministro de acero desde 1983**

[![Website](https://img.shields.io/badge/Website-torresocaranza.cl-2ea44f?style=for-the-badge&logo=google-chrome&logoColor=white)](https://www.torresocaranza.cl)
[![GitHub](https://img.shields.io/badge/GitHub-Organization-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Torres-Ocaranza)
[![Email](https://img.shields.io/badge/Email-info@torresocaranza.cl-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:info@torresocaranza.cl)

*Soluciones internas de software para optimizar nuestros procesos operativos*

</div>

---

## 📊 Estadísticas de la Organización

<div align="center">

![GitHub Org's stars](https://img.shields.io/github/stars/Torres-Ocaranza?style=social)
![GitHub followers](https://img.shields.io/github/followers/Torres-Ocaranza?style=social)
![GitHub repos](https://img.shields.io/badge/Repositorios-16-blue?style=flat-square&logo=github)

</div>

---

## 📋 Sobre la Organización

<div align="center">

**Torres Ocaranza** es una empresa especializada en enfierradura, suministro e instalación de acero para construcción, con más de **39 años de experiencia** en el mercado nacional.

</div>

### 🏭 Capacidad y Infraestructura

- ⚡ **+80.000 toneladas anuales** de capacidad instalada
- 🏗️ **4 plantas** estratégicamente ubicadas en Chile
- 📍 **Cobertura nacional** en proyectos de construcción
- 🎯 **Áreas de negocio**: Minería, Infraestructura, Energía, Salud, Inmobiliario

### 💻 Soluciones Internas

Los repositorios en esta organización contienen **soluciones internas de software** desarrolladas para optimizar nuestros procesos operativos:

- 📄 Gestión documental y certificaciones
- 🚛 Trazabilidad logística y transporte
- 📐 Cubicación de planos
- 🏗️ Sistemas de control de obras
- 📊 Gestión de proyectos industriales

---

## 🚀 Proyectos Principales

### 🔷 Gestión Documental y Certificaciones

<table>
<tr>
<td width="50%">

#### [AceroConexionDossier](https://github.com/Torres-Ocaranza/AceroConexionDossier)
**Aplicación de escritorio** para la carga, validación y gestión de documentos de certificación de calidad en S3.

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![AWS](https://img.shields.io/badge/AWS_S3-FF9900?style=flat-square&logo=amazon-aws&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=flat-square&logo=microsoft-sql-server&logoColor=white)

- ✅ Subida automática de PDFs a AWS S3
- 📊 Trazabilidad completa en SQL Server
- 📧 Notificaciones por correo electrónico
- 🔄 Gestión de estados y reintentos automáticos

</td>
<td width="50%">

#### [Acero-Back-v2](https://github.com/Torres-Ocaranza/Acero-Back-v2)
**Backend en NestJS** para la gestión documental, permisos y certificaciones de proyectos de acero. Integra autenticación JWT, sincronización automática, notificaciones por email y manejo avanzado de archivos S3, conectándose con el frontend Acero-Front-v2.

![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=flat-square&logo=nestjs&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)

- 🔐 Autenticación JWT
- 🔄 Sincronización automática
- 📧 Notificaciones por email
- ☁️ Manejo avanzado de archivos S3
- 🔗 Integración con Acero-Front-v2

</td>
</tr>
<tr>
<td width="50%">

#### [Acero-Front-v2](https://github.com/Torres-Ocaranza/Acero-Front-v2)
**Aplicación frontend en React** para la gestión documental, permisos y certificaciones de proyectos de acero. Integra autenticación, navegación jerárquica y manejo avanzado de archivos, conectándose con el backend Acero-Back-v2.

![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white)

- 🔐 Autenticación integrada
- 🗂️ Navegación jerárquica
- 📁 Manejo avanzado de archivos
- 🔗 Integración con Acero-Back-v2

</td>
<td width="50%">

#### [TransportManager-backend](https://github.com/Torres-Ocaranza/TransportManager-backend)
**Plataforma web** para la gestión y trazabilidad logística de viajes de transporte, incluyendo seguimiento en tiempo real, control de estados, registro documental y recopilación de Guías de Despacho.

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)

- 📍 Seguimiento en tiempo real
- 📊 Control de estados
- 📄 Registro documental
- 📋 Recopilación de Guías de Despacho

</td>
</tr>
</table>

---

### 🚛 Gestión Logística y Transporte

<table>
<tr>
<td width="50%">

#### [TransportManager-frontend](https://github.com/Torres-Ocaranza/TransportManager-frontend)
**Frontend** para la plataforma de gestión logística.

![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)

- 🔗 Integración con TransportManager-backend
- 🌐 Interfaz web moderna

</td>
<td width="50%">

#### [mobiletrackinggps](https://github.com/Torres-Ocaranza/mobiletrackinggps)
**Sistema de seguimiento GPS móvil** para gestión de flota y transporte.

![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white)

- 📍 Seguimiento GPS en tiempo real
- 🚛 Gestión de flota vehicular
- 📊 Monitoreo de rutas

</td>
</tr>
</table>

---

### 🏗️ Sistemas de Gestión de Obras

<div align="center">

| Proyecto | Descripción | Tecnología |
|----------|-------------|------------|
| [CubigestFichaObra2](https://github.com/Torres-Ocaranza/CubigestFichaObra2) | Sistema de gestión de fichas de obra | ![VB.NET](https://img.shields.io/badge/VB.NET-512BD4?style=flat-square&logo=.net&logoColor=white) |
| [OBRACIVIL](https://github.com/Torres-Ocaranza/OBRACIVIL) | Sistema de gestión para obras civiles | ![VB.NET](https://img.shields.io/badge/VB.NET-512BD4?style=flat-square&logo=.net&logoColor=white) |

</div>

---

### ⚙️ Sistemas Especializados

<div align="center">

| Proyecto | Tecnología |
|----------|------------|
| [Optisteel](https://github.com/Torres-Ocaranza/Optisteel) | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) |
| [CUBILINK-CERRILLOS](https://github.com/Torres-Ocaranza/CUBILINK-CERRILLOS) | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) |
| [Metalurgica](https://github.com/Torres-Ocaranza/Metalurgica) | ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white) |
| [EstadosdePagos](https://github.com/Torres-Ocaranza/EstadosdePagos) | ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white) |
| [ControlDimencional](https://github.com/Torres-Ocaranza/ControlDimencional) | ![VB.NET](https://img.shields.io/badge/VB.NET-512BD4?style=flat-square&logo=.net&logoColor=white) |
| [Gestor_OC_Gerdau](https://github.com/Torres-Ocaranza/Gestor_OC_Gerdau) | ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white) |
| [PAGO](https://github.com/Torres-Ocaranza/PAGO) | ![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat-square&logo=css3&logoColor=white) |

</div>

---

## 🛠️ Stack Tecnológico

<div align="center">

### Backend
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E?style=for-the-badge&logo=nestjs&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white)
![VB.NET](https://img.shields.io/badge/VB.NET-512BD4?style=for-the-badge&logo=.net&logoColor=white)

### Frontend
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### Infraestructura
![AWS](https://img.shields.io/badge/AWS_S3-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=for-the-badge&logo=microsoft-sql-server&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

</div>

---

## 📊 Estadísticas

<div align="center">

| Métrica | Valor |
|---------|-------|
| 📦 **Repositorios** | 16 activos (15 privados + 1 público) |
| 🐍 **Python** | Proyectos principales |
| 🔷 **TypeScript/JavaScript** | Backend y Frontend modernos |
| 💎 **C#/VB.NET** | Sistemas especializados |
| 🔒 **Visibilidad** | Repositorios privados (desarrollo interno) |
| 🏭 **Capacidad** | +80.000 toneladas anuales |
| 📅 **Experiencia** | +39 años en el mercado |

</div>

---

## 🔐 Acceso

<div align="center">

⚠️ **15 de los 16 repositorios son privados** y requieren permisos de acceso a la organización.

El único repositorio público es [`.github`](https://github.com/Torres-Ocaranza/.github) que contiene la configuración del perfil de la organización.

Para solicitar acceso a los repositorios privados, contacta con los administradores de la organización.

</div>

---

## 📝 Notas

- ✅ Estos repositorios contienen **soluciones internas** desarrolladas para uso exclusivo de Torres Ocaranza
- 🔄 Los proyectos están en constante desarrollo y mejora
- 📚 Se siguen buenas prácticas de desarrollo y arquitectura limpia
- 📖 Documentación técnica disponible en cada repositorio
- ⚡ Sistemas optimizados para producción y pruebas

---

## 📧 Contacto

<div align="center">

[![Website](https://img.shields.io/badge/Website-www.torresocaranza.cl-2ea44f?style=flat-square&logo=google-chrome&logoColor=white)](https://www.torresocaranza.cl)
[![Email](https://img.shields.io/badge/Email-info@torresocaranza.cl-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:info@torresocaranza.cl)
[![Phone](https://img.shields.io/badge/Teléfono-%2B56%202%202380%209664-25D366?style=flat-square&logo=whatsapp&logoColor=white)](tel:+56223809664)

**Dirección**: Vista Clara #2351, Cerrillos, Santiago, Chile

</div>

### 👥 Equipo de Tecnología

<div align="center">

| Nombre | Cargo | Email |
|--------|-------|-------|
| **Roberto Becerra** | Gerente Informática y Procesos | [![Email](https://img.shields.io/badge/Email-rbecerra@torresocaranza.cl-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:rbecerra@torresocaranza.cl) |
| **René Auger** | Jefe de Soporte e Infraestructura – Desarrollo TI | [![Email](https://img.shields.io/badge/Email-rauger@torresocaranza.cl-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:rauger@torresocaranza.cl) |

</div>

---

<div align="center">

### **Desarrollado con ❤️ por Torres Ocaranza**

*Última actualización: Noviembre 2025*

[![GitHub](https://img.shields.io/badge/GitHub-Torres--Ocaranza-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/Torres-Ocaranza)

</div>
